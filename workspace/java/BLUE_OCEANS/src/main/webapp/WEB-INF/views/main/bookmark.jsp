...
<body>
	<div class="container">
        ...
        <div class="bookmark-container">
            <div id="chart-container-${item}" class="chart-container"></div>
			...
			<label> <input type="button" value="데이터 요청" onclick="test(${item})" />
			</label>
		</c:forEach>
	</div>
...
<script>
    ...
    const chart = LightweightCharts.createChart(document.getElementById(`chart-container-${item}`), {
        layout: {
            textColor: 'black',
	...
	function test(item) {
		let chartInfo = createChart(item); // createChart 함수 호출하여 반환된 정보를 받음
	    ...
	    $.ajax({
	        ...
	        success: function (result) {
	            ...
	            // 가져온 데이터 차트에 등록
	            chartInfo.lineSeries.setData(chartData);
	            chartInfo.timeScale.applyOptions({
	                barSpacing: 10,
	            });
