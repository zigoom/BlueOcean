...
<body>
	...
	<input type="text" id="ui" value="${sessionScope.user}">
	<!-- 유저아이디 값 담아두는 인풋 -->
	...
	<script>
		...
		function handleBookmarkClick() {
			var userId = $("#ui").val();
			var stockName = $("#sn").val();
			var stockCode = $("#sc").val();

			if (bookmarkButton.classList.contains('fa-regular')) {
						...
						success : function(result) {
							...
							if (lastCloseValuePreviousDay) {
								...
								if (lastCloseValue - lastCloseValuePreviousDay >= 0) {
									...
									$('.price-changes-percent').css('color',
											'red');
									$('.last-close-value').css('color', 'red');
									$('#price-change-box').css(
