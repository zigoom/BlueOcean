...
<script>
  ...
  const pagesPerGroup = 10; // 페이지 그룹 당 보여줄 페이지 수
    ...
    $.ajax({
      type : "POST",
      url : "/BlueOcean/admin/loadReply.do",
      async : true,
      dataType : "json",
      data : {},
	...
	// 삭제한 댓글만 불러오는 함수 
	  function deleteOptionLoad() {
	    $.ajax({
	      type : "POST",
	      url : "/BlueOcean/admin/loadReplyOption.do",
	      async : true,
	      dataType : "json",
	      data : {
	    	  replyDelete : 1
	...
	//선택한 댓글 삭제
	  $(".data-tbody").on("click", ".delete-btn", function() {
	    if ($(this).hasClass("btn-primary")) {
	      ...
	      $.ajax({
	        type : "POST",
	        url : "/BlueOcean/admin/deleteReply.do",
	        async : true,
	        dataType : "html",
	        data : {
	        	commentNo : commentNo
	...
	//선택한 댓글 복구
	  $(".data-tbody").on("click", ".recover-btn", function() {
	    if ($(this).hasClass("btn-primary")) {
	      ...
	      $.ajax({
	        type : "POST",
	        url : "/BlueOcean/admin/notDeleteReply.do",
	        async : true,
	        dataType : "html",
	        data : {
	        	commentNo : commentNo
	  ...
	  $(".search-id-btn").on("click", function() {
		    $.ajax({
		      type : "POST",
		      url : "/BlueOcean/admin/loadReplyFromId.do",
		      async : true,
		      dataType : "json",
		      data : {
